...
def reportReadsContentEval(dataObj=None, refParams=None, dirSpec=None):
    ...
    cmd = '''cut -f2 GC_of_raw_flnc_reads.log | {}/distrCurve.R -d -m='GC content of raw flnc reads' -x='Binned GC%' -y='Fraction of reads' -v=50 -w=10 -mainS=28 -p=GC_of_raw_flnc_reads.pdf 2>/dev/null'''.format(utilDir)
    subprocess.call(cmd, shell=True)

    gcAcrossRead(flncFx, "GC_across_raw_flnc_read.log")
    cmd = '''cut -f2- GC_across_raw_flnc_read.log | {}/box.R -stack -nJ -ho=50 -m='GC content across raw flnc reads' -x=Interval -y=GC% -oS=0.5 -w=10 -height=6 -mainS=28 -p=GC_across_raw_flnc_read.pdf 2>/dev/null'''.format(utilDir)
    subprocess.call(cmd, shell=True)
    ...
    subprocess.call(cmd, shell=True)
    cmd = '''{}/distrCurves.R -lgPosX=0.85 -lgPosY=0.9 -x1=0 -x2=10000 -d -x='Binned length (limited in 0-10000)' -w=10 *.lst -b=200 -mainS=28 -p=LengthDistribution.curve.pdf 2>/dev/null'''.format(utilDir)
    subprocess.call(cmd, shell=True)
